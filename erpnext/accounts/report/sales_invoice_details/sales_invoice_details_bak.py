...
class SalesInvoiceDetailsReport(object):
	...
	def append_row(self, row):
		...
		for i, invoice_details in enumerate(invoice_details_list):
			invoice_details.not_billed_amt = invoice_details.amount - invoice_details.billed_amt
			if i == 0:
	...
	def get_invoice_details(self):
		...
		if self.party_type == "Customer":
			...
			for d in si_list:
				if self.invoice_details.get(d.name) != None:
					self.invoice_details.setdefault(d.name, self.invoice_details.get(d.name).append(d))
				else:
	...
	def get_columns(self):
		...
		self.add_column(label=_(self.party_type), fieldname='party',
			fieldtype='Link', options=self.party_type, width=100)
